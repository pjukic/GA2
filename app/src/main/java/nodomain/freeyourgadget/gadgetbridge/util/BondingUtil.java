...
public class BondingUtil {
    ...
    private static long countMax=10;
    ...
    public static BroadcastReceiver getPairingReceiver(final BondingInterface activity) {
        return new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                BluetoothDevice btdevice = activity.getCurrentTarget().getDevice();
                countFound++;
                if(countFound >= countMax) {
                    String toastMsg = "BondingUtil.getPairingReceiver." + countFound + "/" + countMax;
                    GB.toast(toastMsg , Toast.LENGTH_SHORT, GB.ERROR);
                    //BluetoothDevice btdevice= (BluetoothDevice) device;
                    //stopBluetoothBonding(btdevice);
                    //GBDeviceService gbds=new GBDeviceService(this);
                    //gbds.quit();
                    activity.onBondingComplete(false);
                    //return; // stop the endless cycle
                }
                if (GBDevice.ACTION_DEVICE_CHANGED.equals(intent.getAction())) {
                    ...
                    if (activity.getCurrentTarget().getDevice().getAddress().equals(device.getAddress())) {
                        if (device.isInitialized()) {
                            ...
                            LOG.info("===>Device " + device.getName() + " not initialized, notConnecting, notInitializing..");
                        }
                    }
                }
            }
                    ...
                    if (bondingMacAddress != null && bondingMacAddress.equals(device.getAddress())) {
                        ...
                        switch (bondState) {
                            case BluetoothDevice.BOND_BONDED: {
                                ...
                                GB.toast("Bonded with " + device.getAddress() , Toast.LENGTH_SHORT, GB.ERROR);
                            ...
                            case BluetoothDevice.BOND_NONE: {
                                ...
                                GB.toast("Not bonded " + device.getAddress() , Toast.LENGTH_SHORT, GB.ERROR);
                                attemptToFirstConnect(bondingInterface.getCurrentTarget().getDevice());
                                return;
                            }
                            case BluetoothDevice.BOND_BONDING: {
                                LOG.info("===>Bonding in progress with " + device.getAddress());
                                GB.toast("Bonding in progress " + device.getAddress() , Toast.LENGTH_SHORT, GB.ERROR);
                                return;
                            }
                            default: {
                                LOG.warn("===>Unknown bond state for device " + device.getAddress() + ": " + bondState);
                                GB.toast("Unknown bond state " + device.getAddress() , Toast.LENGTH_SHORT, GB.ERROR);
    ...
    private static void connectToGBDevice(GBDevice device) {
        if (device != null) {
            ...
            GB.toast("GBApplication.deviceService().connect...", Toast.LENGTH_SHORT, GB.ERROR);
        } else {
    ...
    public static boolean shouldUseBonding() {
        ...
        ret= prefs.getPreferences().getBoolean(MiBandConst.PREF_MIBAND_SETUP_BT_PAIRING, false);
        GB.toast("BluetootPairing:" + ret, Toast.LENGTH_SHORT,GB.INFO);
        return ret;
    }
    ...
    public static void tryBondThenComplete(BondingInterface bondingInterface, GBDeviceCandidate deviceCandidate) {
        ...
        if (bondState == BluetoothDevice.BOND_BONDED) {
            ...
            GB.toast(bondingInterface.getContext(), bondingInterface.getContext().getString(R.string.pairing_in_progress, device.getName(), device.getAddress()), Toast.LENGTH_SHORT, GB.INFO);
            return;
        }

        GB.toast(bondingInterface.getContext(), bondingInterface.getContext().getString(R.string.pairing_creating_bond_with, device.getName(), device.getAddress()), Toast.LENGTH_SHORT, GB.INFO);
