...
public abstract class AbstractFetchOperation extends AbstractHuamiOperation {
    ...
    protected String getLastDBSyncTimeKey(){return null;};
    ...
    void saveLastSyncTimestamp(@NonNull GregorianCalendar timestamp) {
        ...
        LOG.info("===>saveLastSyncTimestamp - " + DateTimeUtils.formatDateTime(timestamp.getTime()));
        editor.apply();
        //PJ
        LOG.info("===> editor.toString:" + editor.toString());
        LOG.info("===> getDeviceSpecificSharedPrefs.getAll: " + GBApplication.getDeviceSpecificSharedPrefs(getDevice().getAddress()).getAll());
    }

    void saveLastDBSyncTimestamp(@NonNull GregorianCalendar timestamp) {
        SharedPreferences.Editor editor = GBApplication.getDeviceSpecificSharedPrefs(getDevice().getAddress()).edit();
        editor.putLong(getLastDBSyncTimeKey(), timestamp.getTimeInMillis());
        LOG.info("===>saveLastDBSyncTimestamp - " + DateTimeUtils.formatDateTime(timestamp.getTime()));
        editor.apply();
    }


    long getLastDBSuccessfulSyncTimeStampMillis() {
        long dbTimeStampMillis = GBApplication.getDeviceSpecificSharedPrefs(getDevice().getAddress()).getLong(getLastDBSyncTimeKey(), 0);
        if(dbTimeStampMillis != 0) {
            return dbTimeStampMillis;
        }
        return 0;
    }
    ...
    protected GregorianCalendar getLastSuccessfulSyncTime() {
        ...
        if (timeStampMillis != 0) {
            ...
            LOG.info("===>getLastSuccesfulSynctime.1orig - " + DateTimeUtils.formatDateTime(calendar.getTime()));
            if(fetchCount==1){
                //try to find a dbLastSuccessfulSynctime

                long dbTimeStampMillis = GBApplication.getLastDBSyncTimestamp("lastDBSyncTimeMillis");
                if (dbTimeStampMillis != 0 && dbTimeStampMillis <= timeStampMillis) {
                    calendar.setTimeInMillis(dbTimeStampMillis);
                    LOG.info("===>getLastSuccesfulSynctime.dbSyncTinme - " + DateTimeUtils.formatDateTime(calendar.getTime()));
                } else {
                    GregorianCalendar calendar_minus_10 = BLETypeConversions.createCalendar();
                    calendar_minus_10.add(Calendar.DAY_OF_MONTH,-10);
                    if(timeStampMillis < calendar_minus_10.getTimeInMillis()) {
                        //the earlier time
                        calendar.setTimeInMillis(timeStampMillis);
                    } else {
                        //calendar.add(Calendar.DAY_OF_MONTH, -10);
                        calendar=calendar_minus_10;
                    }
                }
        ...
        LOG.info("===>getLastSuccesfulSynctime._minus20days - " + DateTimeUtils.formatDateTime(calendar.getTime()));
