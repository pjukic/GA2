...
//            case TYPE_UNKNOWN: // fall through
    ...
    @Override
    public void addGBActivitySample(MiBandActivitySample activitySample, String token) {
        getSampleDao().insertOrReplace(activitySample);
        GAServices.postSampleToServer(activitySample, token);
    }

    @Override
    public void addGBActivitySamples(MiBandActivitySample[] activitySamples, String token) {
        getSampleDao().insertOrReplaceInTx(activitySamples);
        GAServices.postMultipleSamplesToServer(activitySamples, token);
    }


    public void addGBActivitySampleX(MiBandActivitySample activitySample, String token, GregorianCalendar timestamp) {
        getSampleDao().insertOrReplace(activitySample);
        GAServices.postSampleToServer(activitySample, token, timestamp);
    }


    public void addGBActivitySamplesX(MiBandActivitySample[] activitySamples, String token, GregorianCalendar timestamp) {
        getSampleDao().insertOrReplaceInTx(activitySamples);
        GAServices.postMultipleSamplesToServer(activitySamples, token, timestamp);
    }
