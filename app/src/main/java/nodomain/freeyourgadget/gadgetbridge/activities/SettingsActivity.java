...
public class SettingsActivity extends AbstractSettingsActivity {
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        pref = findPreference("auto_export_interval_guardian");
        pref.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
            @Override
            public boolean onPreferenceChange(Preference preference, Object autoExportInterval) {
                String summary = String.format(
                        getApplicationContext().getString(R.string.pref_auto_export_guardian_summary),
                        Integer.valueOf((String) autoExportInterval));
                preference.setSummary(summary);
                PeriodicGuardianExporter.sheduleAlarm(getApplicationContext(), GBApplication
                        .getPrefs().getInt(GBPrefs.AUTO_GUARDIAN_EXPORT_INTERVAL, 3), true);
                return true;
            }
        });

        autoFetchInterval = GBApplication.getPrefs().getInt("auto_export_interval_guardian", 3);
        summary = String.format(
                getApplicationContext().getString(R.string.pref_auto_export_guardian_summary),
                autoFetchInterval);
        pref.setSummary(summary);
