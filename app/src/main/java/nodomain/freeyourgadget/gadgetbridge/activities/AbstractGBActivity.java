        ...
        if (GBApplication.isDarkThemeEnabled()) {
            if ((flags & NO_ACTIONBAR) != 0) {
                if (GBApplication.isAmoledBlackEnabled())
                    ...
                    activity.setTheme(R.style.GuardianThemeDark_NoActionBar);
            } else {
                if (GBApplication.isAmoledBlackEnabled())
                    activity.setTheme(R.style.GadgetbridgeThemeBlack);
                else
                    activity.setTheme(R.style.GuardianThemeDark);
            }
        } else {
            if ((flags & NO_ACTIONBAR) != 0) {
                activity.setTheme(R.style.GuardianTheme_NoActionBar);
            } else {
                activity.setTheme(R.style.GuardianTheme);
            }
        }
